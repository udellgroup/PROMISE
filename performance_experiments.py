...
def get_experiments(data, model_type, model_params, opt, precond_type, hyperparams, max_epochs, tol, min_loss, bg, bh):
    if opt in ['sgd', 'svrg', 'saga', 'lkatyusha', 'slbfgs']:
        ...
        # Get the experiments
        experiments = [Experiment(data, model_type, model_params, opt, opt_params, tol, min_loss) for opt_params in params_list]
        ...
        experiments = [Experiment(data, model_type, model_params, opt, opt_params, tol, min_loss)]
...
def main():
    ...
    parser = argparse.ArgumentParser(description = 'Run experiments for performance plots. We solve problems to within 1e-4 of the optimum provided by running sklearn_opt.py. \
                                    ...
                                    Random seeds are set according to SEEDS in constants.py. These should be the same as the seeds used in sklearn_opt.py.')
    ...
    parser.add_argument('--min_loc', type = str, required = True, help = 'Location of the optimal solution (should be a .csv file)') # Path to the optimal solution
    ...
    epochs = args.epochs
    mu_unscaled = args.mu
    min_loc = os.path.abspath(args.min_loc)
    ...
    print(f"Location of optimal solution: {min_loc}")
    ...
    # Get minimum from min_loc
    df = pd.read_csv(min_loc)
    ...
    experiments = get_experiments(data, problem_type, model_params, opt, precond_type, hyperparams, epochs, TOLERANCE, min_loss, BATCH_SIZE, bh)
